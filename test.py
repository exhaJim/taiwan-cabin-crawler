...
class TestJmlScraper(unittest.TestCase):
    ...
    def test_get_data(self):
        data = self.scraper.get_data('2024-03-01', format='pandas')
        self.assertIsInstance(data, pd.DataFrame)

        data = self.scraper.get_data('2024-03', format='list')
        self.assertIsInstance(data, dict)


class TestKuaiguScraper(unittest.TestCase):
    def setUp(self):
        self.scraper = KuaiguScraper(year=2024, month=3)
